...
#
# batchcorder is a headless Rust+Python library: a PyO3 extension built into
# the venv with maturin. We need a C toolchain (to link the extension), uv,
# and rustup (edition 2024 / let chains require Rust >= 1.85; rust-toolchain.toml
# pins channel = stable, so the exact toolchain auto-resolves on first cargo run).
set -euo pipefail
