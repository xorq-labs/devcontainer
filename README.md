...
6. **`Dockerfile`** (classic base — seed overlays and `DEV_NIX_BASE=0`) — exposes build args you can override from `compose.override.yml` rather than editing the Dockerfile in place: `BASE_IMAGE` (default `mcr.microsoft.com/devcontainers/python:3.12-bookworm`) for non-Python base images; `EXTRA_PATH` (empty by default, set to the Python venv's bin dir in the project override) prepended to the container `PATH` so project tools resolve; and tool-version pins (`NODE_MAJOR`, `JUST_VERSION`, `SOPS_VERSION`, `CLAUDE_CODE_VERSION`) — bump these together with their companion checksum args where present (`NODESOURCE_SHA256`, `JUST_SHA256`, `SOPS_SHA256`; Claude Code is installed via npm and has no checksum arg). `nix/base/Dockerfile.nix-default` honors the same `EXTRA_PATH` contract on the Nix-base path. To bump the Claude Code pin, run `devcontainer bump-claude-code` — it updates `CLAUDE_CODE_VERSION` *and* the Nix base's `pkgs/claude-code.nix` together (pass an explicit version to pin it, or `--check` to compare without editing); `tests/test-claude-code-pin-sync.sh` guards the two against drift. Likewise, run `devcontainer bump-nix` to bump the Nix pin in `lib/nix-seed.sh` — it rewrites the coupled `NIX_VERSION` and `NIX_INSTALLER_SHA256` defaults together (downloading the target installer to compute its checksum), and accepts an explicit version or `--check`. Its "latest" is the highest stable upstream **tag** — NixOS/nix publishes no GitHub releases — and it refuses to resolve backwards, so a truncated tag listing fails loudly instead of quietly pinning an older version. And `devcontainer bump-hadolint` does the same for the hadolint pin in `projects/devcontainer/install-system.sh`, which is a coupled *triple* — `HADOLINT_VERSION` plus `HADOLINT_SHA256_AMD64` and `HADOLINT_SHA256_ARM64` — and also bumps the paired hadolint `rev:` in `.pre-commit-config.yaml`, which `tests/test-lint-config-sync.sh` requires to match. It downloads both arch binaries before touching anything, so a half-bump (notably a stale arm64 sha, which breaks the arm64 container build at `sha256sum -c` while every test stays green) cannot be written. It has two read-only flags, and they answer different questions: `--check` is a **report** — current vs latest, no download, exit 0 whether or not the versions match (drift is news, not a failure; it still exits non-zero when it cannot resolve a version to report at all), the same shape as `bump-nix --check` and `bump-claude-code --check`; `--verify` is a **gate** — it fetches the release for the currently committed version (or an explicit one) and checks *both* committed per-arch checksums against it, exiting non-zero on a mismatch and also on a failed fetch, since a gate that passes when it could not check is worse than no gate. The two are mutually exclusive. Finally, `devcontainer bump-nix-base` moves the fourth pin — the published Nix base digest in `nix/base/compose.nix-base.yml`, which is what every non-seed overlay actually builds on. It resolves the digest straight from ghcr (anonymously, so it needs only `curl` — no docker, no `read:packages`), accepts a tag or an explicit digest, and carries the same `--check` report / `--verify` gate split as `bump-hadolint`. Prefer it to a hand edit for one specific reason: it refuses to pin anything that is not a multi-arch **manifest list**, and `imagetools inspect` prints the per-arch digests directly beneath the list digest — so pinning `sha-<short>-amd64` by mistake is easy, looks entirely correct on an amd64 laptop, and breaks every arm64 consumer at pull time.
...
Two worktree paths are hardcoded in `dev/setup-worktree` rather than living in `worktree-{symlinks,copies}.txt`: `.gitignore` is always copied (git opens it with `O_NOFOLLOW`, so a symlink would `ELOOP`), and `.claude` is always set up as a **real directory with its two state subdirectories symlinked** — `container-audit/` and `container-sessions/` point at the main checkout, because audit logs and session captures are devcontainer infrastructure that must aggregate there regardless of project, while the rest of `.claude` (tracked content like `agents/`, and per-worktree `settings.local.json`) stays local to the worktree. Worktrees created before this layout carry a whole-directory `.claude` symlink; setup-worktree leaves those as-is (migrate by deleting the symlink and re-running it). See [docs/adr/0003-track-claude-agents-in-git.md](docs/adr/0003-track-claude-agents-in-git.md).
...
- **Every `gh` command fails with "cowardly refusing to continue with multi account migration … dbus-launch"** — the container's `hosts.yml` has no token and gh refuses at config load, so even `gh --version` dies. Re-enter from the host and `setup_gh` fills it in; even when it can't (and warns that it copied nothing), it removes a stale tokenless copy from the container, which puts `gh` back in business via `GH_TOKEN`. To get the full config bridged, `gh auth login` on the **host** and re-enter.
...
> [!NOTE]
> In a worktree, `.claude` is a real directory, but `.claude/container-audit` and `.claude/container-sessions` are symlinks to the main checkout's copies (`dev/setup-worktree` always links those two so audit logs and session captures aggregate in one place; pre-existing worktrees may still have the older whole-directory `.claude` symlink, which resolves the same way). The audit log and session stubs are therefore **shared across all worktrees** — `devcontainer audit` reports every worktree's activity, and `devcontainer clean` (or `audit --clear`) run from *any* worktree clears them for all. Everything else under `.claude` is per-worktree, including `settings.local.json`.
