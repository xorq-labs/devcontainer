...
- **Every `gh` command fails with "cowardly refusing to continue with multi account migration … dbus-launch"** — the container's `hosts.yml` has no token in it (gh keeps the token in the host's keyring; the container has none). The refusal is at config load, so even `gh --version` dies and `GH_TOKEN` is not consulted. `setup_gh` fills the token in, so re-entering from the host overwrites the bad file. If the host can't produce one it copies nothing and says so on stderr, leaving the stale file: `rm ~/.config/gh/hosts.yml` in the container, then `gh auth login` on the **host** or export `GH_TOKEN`.
