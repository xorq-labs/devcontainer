...
#   an already-chowned volume later (e.g. by a `dc exec -u root`). The mount
#   point stays user-owned, so the recursion is skipped. Forcing a full pass
#   there is a separate, explicit operation — not something every cold start
#   should pay for.
#
...
# dev_volume_chown_needed <mount-point> <owner> — true (0) when <mount-point>
# needs the recursive chown: it exists and is not owned by <owner>. An
# unstattable directory is treated as needing it (let chown report the real
# error); a missing mount point needs nothing.
dev_volume_chown_needed() {
    [ -d "$1" ] || return 1
    _vp_owner="$(stat -c %U "$1" 2>/dev/null)" || return 0
    [ "$_vp_owner" = "$2" ] && return 1
    return 0
}

# dev_chown_volume_targets <owner> <home-prefix> <mount-point>... — chown each
# mount point (recursively, guarded) and its <home-prefix>-scoped ancestors to
# <owner>:<owner>. Ancestors are walked upward only while still strictly under
# <home-prefix>, so <home-prefix> itself and anything outside it (notably the
# bind-mounted host workspace) are never touched.
#
...
dev_chown_volume_targets() {
    ...
    for _vp_d in "$@"; do
        ...
        if dev_volume_chown_needed "$_vp_d" "$_vp_user"; then
            chown -R "$_vp_user:$_vp_user" "$_vp_d" 2>/dev/null || true
        fi
        ...
        while [ "$_vp_p" != "/" ] && [ "$_vp_p" != "." ]; do
            ...
            chown "$_vp_user:$_vp_user" "$_vp_p" 2>/dev/null || true
            _vp_p="$(dirname "$_vp_p")"
        done
    done
}
