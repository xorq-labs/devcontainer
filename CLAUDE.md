...
- A mutation record states what the run MEASURED, not what you expected: the red count, the assertion names, and — per mutation, because it is not uniform — whether it was already caught before your change. Every one of those fields has been recorded wrong here and corrected in review. `tests/mutation-coverage <suite> '<cmd>'...` runs mutations in throwaway copies, reports no-op mutations (a silently non-matching `sed` otherwise reads as a passing run that tested nothing), and lists the assertions no mutation reds — a worklist, since ADR-0005 proves the suite has teeth without saying anything about the individual assertions.
- Assert on line-structured output with `assert_line`/`assert_no_line`, not `assert_contains`/`assert_not_contains`: a substring needle also matches a needle that is a PREFIX of a longer line, which has shipped a fail-open assertion here. Reserve the substring pair for genuinely unstructured haystacks, and give every negative assertion a paired positive — a negative alone also passes when the code under test did nothing (`tests/test-harness-assertions.sh` pins the difference).
