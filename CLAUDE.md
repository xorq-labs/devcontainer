...
- A source-build pin is not always only hashes (ADR-0007's 2026-08-20
  ...
  and all silent: a key with no file, a file with no key (regenerated by
  nothing, so it rots against the rev beside it), and a key whose file
  `source-build.nix` never imports (the derivation quietly keeps embedding a
  stub). The bun.lock rewrite that makes bun2nix parse a `github:` dependency
  at all (`degrade_git_lock_entries`) is a fourth: bun2nix dispatches on entry
  arity and bun writes `github:` deps at npm arity, so without the rewrite the
  generated expression fetches a registry URL that does not exist
  (`test: tests/test-bump-kenn.sh` §6 — shape agreement plus the rewrite
  itself, driven against a canned bun.lock; `tool: nix/kenn/update.py`, whose
  `BOGUS_REGISTRY_URL_RE` refuses to WRITE an expression where the rewrite
  missed, since the alternative symptom is a 404 far from its cause).
  Whether a committed generated file still matches its rev is `tool:` like the
  hashes — the build failing is the oracle, and it does fail: a stale
  `bun.nix` makes the offline `bun install` miss packages rather than silently
  succeed.
